...
tokenizer.prototype.tokenize_and_insert = function( data ) {
  ...
  while( bpos < wb.length ) {
    ...
    if( /<</.test(work) && !(/>>/.test(work))) {
      ...
      ms.push( { type: 'keyvalue', value: { key: b, value: lb }});
      ...
      if( rs[0].toLowerCase() == 'include' &&
        ...
        if( rs.slice(1)[0].trim()[0] == '"' ) {
          ms.push( { type: 'include', value: get_string(rs.slice(1)[0].trim()) } );
        } else {
          ms.push( { type: 'include', value: rs.slice(1)[0].trim() } );
        }
      } else {
        // properly do this
        temp_buff = temp_buff.replace(rs[0].trim(), '');

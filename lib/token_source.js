...
tokenizer.prototype.build_stack = function() {
  ...
  while( bpos < wb.length ) {
    ...
    if( /<</.test(work) && !(/>>/.test(work))) {
      ...
      self.token_stack.push( { type: 'keyvalue', value: { key: kill_ws(t[0]), value: lb }});
      ...
      // form of quote
      var rs;
      work = work.match(/^\s*([^#]+)(?:\s*#.*)?$/)[1];
      if( work.slice(-1) == '\\' ) {
        if( temp_buff !== undefined )
          temp_buff = temp_buff + " " + kill_ws(work.slice(-1));
        else
          temp_buff = kill_ws(work.slice(-1));
        break;
      } else if( temp_buff !== undefined && temp_buff !== "" ) {

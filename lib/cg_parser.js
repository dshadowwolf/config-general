...
function cfgparser(opts,ki) {
  ...
  var tfps = self.options.MergeDuplicateBlocks===undefined?false:self.options.MergeDuplicateBlocks;

  this.known_includes = ki!==undefined?ki:{};
  this.symbols = new symtab(undefined,'toplevel',tfps);
  this.blocks = [];
  this.curblock = {};
  this.bns = [];
  this.cbn = "";
  ...
  this.token_source.on('includereq', function(data) {
    ...
    flist.forEach( function( e, i, a ) {
      ...
      if( fs.existsSync(e) ) {
        var p = fs.readFileSync( e, 'utf8' ).split("\n");
...
cfgparser.prototype.putValue = function(key,value) {
  ...
  if( this.options.FlagBits !== undefined ) {
    if( key in this.options.FlagBits ) {
      workval = doFlagBits(key,value,this.options.FlagBits);
    }
  }

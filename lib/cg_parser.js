...
function cfgparser(opts,ki) {
  ...
  this.token_source.on('includereq', function(data) {
    ...
    var flist = self.generate_flist(data);
    var tlist;
    var extra;
  ...
  this.generate_flist = function(data) {
    var self = this;
    var flist = [];
    var base_name;
    ...
    if( self.options.IncludeRelative ) {
      ...
      var configPaths = [ path.dirname(base_name) ];
      if (typeof(self.options.ConfigPath) !== 'undefined' ) {
        for (var i=0; i < self.options.ConfigPath.length; i++) {
            configPaths.push( self.options.ConfigPath[i] );
        }
      }
      configPaths.forEach(
          function(dir,i,a) {
...
cfgparser.prototype.maybeLower = function(s) {
  if( this.options.LowerCaseNames === true )
...
cfgparser.prototype.newBlock = function(d) {
  var name = this.maybeLower(this.interpolate(d.name));
  this.pushblock();
...
cfgparser.prototype.closeBlock = function(d) {
  var work = this.maybeLower(this.interpolate(d));

  this.exitScope();

  if( work.toLowerCase() != this.cbn.toLowerCase() ) {
    throw new Error( 'mis-nested blocks. <' + work + '> followed by </' + this.cbn + '>' );
  }
...
cfgparser.prototype.putBlock = function(name, block) {
  if( this.curblock[name] !== undefined ) {
    ...
    if( Array.isArray(this.curblock[name]) )
      if( 'object' != typeof this.curblock[name][0] )
        throw new Error("Attempt to write a block over a non-block value");
